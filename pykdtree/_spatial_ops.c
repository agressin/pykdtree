...
************************************************/
void scatter_reduce_float(uint64_t *indices, float *values,
                              uint64_t n, uint64_t n_bins, uint64_t n_cols,
                              int method,
                              float *output, uint64_t *count_out)
{
    int64_t i;
    int64_t local_n = (int64_t)n;
    uint64_t j;
    ...
    uint64_t bin_bytes = n_bins * n_cols * sizeof(float);
    ...
    if (use_parallel)
    {
        ...
        float **local_out = (float **)malloc(n_threads * sizeof(float *));
        ...
        {
            local_out[t] = (float *)malloc(n_bins * n_cols * sizeof(float));
            if (method == 1) /* min: init to +inf */
                for (j = 0; j < n_bins * n_cols; j++)
                    local_out[t][j] = DIST_MAX_float;
            else if (method == 2) /* max: init to -inf */
                for (j = 0; j < n_bins * n_cols; j++)
                    local_out[t][j] = -DIST_MAX_float;
            else /* sum/mean: init to 0 */
                memset(local_out[t], 0, n_bins * n_cols * sizeof(float));
...
Params:
    ...
    count_out   : (n_bins,) count per bin (for mean; NULL if not needed)
************************************************/
void scatter_reduce_double(uint64_t *indices, double *values,
                              uint64_t n, uint64_t n_bins, uint64_t n_cols,
                              int method,
                              double *output, uint64_t *count_out)
{
    int64_t i;
    int64_t local_n = (int64_t)n;
    uint64_t j;
    ...
    uint64_t bin_bytes = n_bins * n_cols * sizeof(double);
    /* Use parallel reduction if we can afford thread-local copies (< 64 MB total) */
    int use_parallel = (n_threads > 1) && ((uint64_t)n_threads * bin_bytes < 64ULL * 1024 * 1024);
    ...
    {
        /* Allocate thread-local accumulators */
        double **local_out = (double **)malloc(n_threads * sizeof(double *));
        uint64_t **local_cnt = NULL;
        if (method == 3 || count_out)
            local_cnt = (uint64_t **)malloc(n_threads * sizeof(uint64_t *));
        ...
        for (t = 0; t < n_threads; t++)
        {
            local_out[t] = (double *)malloc(n_bins * n_cols * sizeof(double));
            if (method == 1) /* min: init to +inf */
                for (j = 0; j < n_bins * n_cols; j++)
                    local_out[t][j] = DIST_MAX_double;
            else if (method == 2) /* max: init to -inf */
                for (j = 0; j < n_bins * n_cols; j++)
                    local_out[t][j] = -DIST_MAX_double;
            else /* sum/mean: init to 0 */
                memset(local_out[t], 0, n_bins * n_cols * sizeof(double));
